...
walk(sourceRoot).forEach((filePath) => {
    ...
    traverse(ast, {
        JSXElement(elementPath) {
            ...
            if(element.openingElement.name.type !== "JSXIdentifier" || element.openingElement.name.name !== "MythicCluster"){
                return;
            }
            const parentClasses = staticClasses(element);
            if(!parentClasses.includes("mythic-search-result-inline")){ return; }
            ...
            if(!staticClasses(labelElement).includes("mythic-search-result-label")
                || !staticClasses(valueElement).includes("mythic-search-result-value")){
                return;
            }
            ...
            edits.push({
                ...
                value: `<MythicMetadataItem className="mythic-search-result-inline" density="compact" layout="inline" ${labelProp}>${valueSource}</MythicMetadataItem>`,
            });
            migratedItems += 1;
            elementPath.skip();
        },
...
console.log(`${write ? "Migrated" : "Would migrate"} ${migratedItems} search metadata items in ${changedFiles} files.`);
